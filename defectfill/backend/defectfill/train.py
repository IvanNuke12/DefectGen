...
from utils import save_checkpoint, load_checkpoint, read_checkpoint_config
# TensorBoard support
...
def write_status(output_dir, **kwargs):
    """Lightweight JSON status file used by the web UI to poll training progress.
    This does not affect training logic in any way, it just persists the latest
    known metrics so the FastAPI backend can read them without parsing stdout."""
    status_path = os.path.join(output_dir, "status.json")
    payload = {"updated_at": datetime.datetime.now().isoformat(), **kwargs}
...
def train(args):
    ...
    log_file.write(f"Object class: {args.class_name} (project: {args.object_class})\n")
    ...
    # When resuming, build the model with the SAME LoRA config used when the
    # checkpoint was trained, otherwise loading fails with a shape mismatch.
    if args.resume_from:
        ckpt_cfg = read_checkpoint_config(args.resume_from)
        if ckpt_cfg and ckpt_cfg.get("lora_rank") is not None:
            if ckpt_cfg["lora_rank"] != args.lora_rank:
                print(f"[Info] Resuming checkpoint trained with lora_rank={ckpt_cfg['lora_rank']} "
        ...
        if ckpt_cfg and ckpt_cfg.get("lora_alpha") is not None:
            if ckpt_cfg["lora_alpha"] != args.lora_alpha:
                print(f"[Info] Resuming checkpoint trained with lora_alpha={ckpt_cfg['lora_alpha']} "
    ...
    write_status(
        args.output_dir, state="running", step=global_step, total_steps=total_steps,
    ...
    while global_step < total_steps:
        for batch in train_loader:
            ...
            object_classes = [args.class_name for _ in defect_samples]
            ...
            if accumulation_step >= args.gradient_accumulation_steps:
                ...
                # Web UI progress reporting (every step, cheap to write)
                elapsed = time.time() - train_start_time
                steps_done = max(1, global_step - start_step)
                secs_per_step = elapsed / steps_done
                eta_seconds = secs_per_step * (total_steps - global_step)
                write_status(
                    args.output_dir,
                    state="running",
                    step=global_step,
                    total_steps=total_steps,
                    loss_defect=defect_loss.item(),
                    loss_object=object_loss.item(),
                    loss_attention=attention_loss.item(),
                    loss_total=total_loss.item() * args.gradient_accumulation_steps,
                ...
                # Progreso visible en la consola web: write_status alimenta la
                # barra de progreso, y este print alimenta el panel de logs
                # (que de otro modo parecía "congelado" porque no llegaba nada).
                log_file.write(
                    f"Step {global_step}/{total_steps} | "
                    f"defect {defect_loss.item():.4f} | obj {object_loss.item():.4f} | "
                    f"attn {attention_loss.item():.4f} | loss {total_loss.item():.4f} | "
                ...
                if global_step % args.save_steps == 0 and global_step != total_steps:
    ...
    write_status(
        args.output_dir, state="completed", step=global_step, total_steps=total_steps,
        object_class=args.object_class, defect_type=args.defect_type,
